(ns motomfg.advisor
  "MotoAdvisor -- the *contained intelligence node* for the motorcycle-
  manufacturing plant-operations coordination actor.

  It normalizes production-batch patches (product-category/engine-
  displacement/quantity/weld-defect-rate), drafts a welding/assembly/
  test-bench maintenance scheduling proposal against a piece of
  equipment, drafts a safety-concern flag, and drafts an outbound
  motorcycle shipment coordination proposal against a production
  batch. CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real welding/assembly/test-bench-
  equipment actuation, freight dispatch, or ECE R78/ECE R40 motorcycle
  type-approval certification -- see README `What this actor does NOT
  do`. Every output is censored downstream by `motomfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct welding/
  ...
  contract every caller of this actor agrees to) -- `motomfg.governor`
...
(defn- log-production-batch
  ...
  validates the patch; it does not invent the batch's product-
  category, engine-displacement, quantity, weld-defect-rate or
...
(defn- schedule-maintenance
  "Draft a welding/assembly/test-bench maintenance-window scheduling
  ...
  `motomfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a frame-weld-defect/brake-safety/emissions-compliance
  ...
  low-stakes, and it is never gated on the referenced equipment/batch
  being verified (a concern can be raised about ANY equipment or
  batch, verified or not -- see README `What this actor does NOT do`
  re: never blocking safety-relevant reporting on an administrative
  technicality). See `motomfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `motomfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound motorcycle shipment coordination proposal against
  a production batch. The advisor passes through the caller's own
  claimed unit quantity -- it does NOT invent one, and
  `motomfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたはオートバイ製造工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "溶接・組立・検査治具等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "ECE R78/R40 二輪車型式認証(type-approval)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `motomfg.governor`
