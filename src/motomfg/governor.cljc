(ns motomfg.governor
  "Motorcycle Plant Operations Governor -- the independent compliance
  layer that earns the MotoAdvisor the right to commit. The advisor
  has no notion of whether a piece of equipment it wants to schedule
  maintenance against has actually been inspected/registered, whether
  a batch it wants to coordinate a shipment against has actually been
  QC-verified/registered, whether a maintenance proposal secretly
  tries to ACTUATE (rather than merely draft-schedule) welding/
  assembly/test-bench equipment, whether a proposal secretly tries to
  self-issue an ECE R78 (motorcycle brake system) / ECE R40
  (motorcycle emissions) type-approval CERTIFICATION (an authority
  this actor never holds), whether a shipment proposal's own claimed
  quantity would blow through the batch's own logged production
  quantity, or when an act stops being a coordination proposal and
  becomes direct welding/assembly/test-bench-equipment control, so
  this MUST be a separate system able to *reject* a proposal and fall
  back to HOLD.

  `:itonami.blueprint/governor` is
  `:motorcycle-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:welder/actuate` or
                                       `:press/run`) is the 'direct
                                       welding/assembly/test-bench-
   ...
   10. Invalid product-category    -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`motomfg.registry/product-
                                       category-valid?`), the batch
                                       record is rejected rather than
                                       let a fabricated category
                                       through.
   11. Invalid engine-displacement -- for `:log-production-batch`, if
                                       the patch declares an
                                       `:engine-displacement-cc` that
                                       is not a physically plausible
                                       rating (`motomfg.registry/
                                       engine-displacement-valid?`),
                                       the batch record is rejected
                                       rather than let a fabricated/
                                       sensor-error rating through.
   12. Invalid weld-defect-rate    -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:weld-defect-rate-percent` that
                                       is not a physically plausible
                                       reading (`motomfg.registry/
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [motomfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct welding/
  assembly/test-bench-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct welding/assembly/test-bench-equipment control,
  a fabricated actuation effect) is this actor's central scope
  boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は溶接・組立・検査治具等の設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate welding/assembly/test-bench equipment -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "溶接・組立・検査治具等の設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue an ECE R78 (motorcycle brake system) / ECE R40
  (motorcycle emissions) type-approval certification mark -- an
  authority exclusively reserved to the accredited type-approval body,
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "ECE R78/R40 二輪車型式認証(type-approval)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-engine-displacement-violations
  "For `:log-production-batch`, if the patch declares an
  `:engine-displacement-cc` that is not a physically plausible rating,
  reject rather than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [ed (:engine-displacement-cc (:value proposal))]
      (when (and (some? ed) (not (registry/engine-displacement-valid? ed)))
        [{:rule :invalid-engine-displacement
          :detail (str ed " は物理的に妥当な engine-displacement-cc の範囲外")}]))))

(defn- invalid-defect-rate-violations
  "For `:log-production-batch`, if the patch declares a
  `:weld-defect-rate-percent` that is not a physically plausible
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rr (:weld-defect-rate-percent (:value proposal))]
      (when (and (some? rr) (not (registry/defect-rate-valid? rr)))
        [{:rule :invalid-defect-rate
          :detail (str rr "% は物理的に妥当な不良率の範囲外")}]))))

(defn check
  "Censors a MotoAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-engine-displacement-violations request proposal)
