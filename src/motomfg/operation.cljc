(ns motomfg.operation
  "MotoOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MotoAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Motorcycle Plant
...
(defn build
  "Compiles a MotoOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `motomfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MotoAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Motorcycle Plant Operations Governor -- independent censor.
      (g/add-node :govern
