(ns motomfg.store
  "SSoT for the motorcycle-manufacturing plant-operations coordination
  actor, behind a `Store` protocol so the backend is a swap, not a
  rewrite -- the same seam every `cloud-itonami-isic-*` actor in this
  fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-3092`'s own
  `bikemfg.store`, `cloud-itonami-isic-2211`'s own `tyremfg.store`),
  this build ships a single `MemStore` backend only (atom of EDN) --
  the deterministic default for dev/tests/demo, no deps. Per
  docs/adr/0001-architecture.md Decision 1, this vertical is
  self-contained (no external motorcycle-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-category/engine-displacement/
                             quantity/weld-defect-rate record.
    ...
    - `equipment`         -- a welding/assembly/test-bench unit's own
    ...
    - `maintenance`       -- a scheduled welding/assembly/test-bench
                             maintenance-window DRAFT against a piece
                             of equipment (`motomfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound motorcycle shipment
                             DRAFT (`motomfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (motomfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (motomfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :sport-motorcycle
                :model-code "GX-600R" :engine-displacement-cc 600.0
                :quantity-units 500.0 :weld-defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :scooter
                :model-code "CS-125" :engine-displacement-cc 125.0
                :quantity-units 80.0 :weld-defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :cruiser-motorcycle
                :model-code "CR-1200" :engine-displacement-cc 1200.0
                :quantity-units 600.0 :weld-defect-rate-percent 2.3
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"framewelder-001" {:id "framewelder-001" :kind :frame-welding-robot
                       :verified? true :registered? true
                       :last-maintenance-date "2026-05-01"}
   "dynobench-002" {:id "dynobench-002" :kind :emissions-dyno-test-bench
                     :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `motomfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `motomfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- one verified+registered sport-motorcycle batch with shipping
  headroom (schedulable), one verified+registered scooter batch that
  is nearly fully shipped (a small new shipment blows through its own
  logged production quantity -- HARD hold), one UNVERIFIED/unregistered
  batch (blocks any shipment coordinated against it); one
  verified+registered frame-welding-robot unit (schedulable for
  maintenance), one UNVERIFIED/unregistered emissions-dyno-test-bench
  unit (blocks any maintenance scheduling against it) -- so the actor +
