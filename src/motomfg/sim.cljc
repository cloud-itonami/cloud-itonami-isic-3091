(ns motomfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE welding/
  assembly/test-bench equipment directly (permanently blocked, no
  override), a proposal that tries to self-issue an ECE R78/ECE R40
  motorcycle type-approval certification (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product category, a
  production-batch patch with an implausible engine-displacement, and
  a production-batch patch with an implausible weld-defect-rate
  reading.
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-category :sport-motorcycle :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on framewelder-001 (verified, registered, frame-welding robot -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on framewelder-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on dynobench-002 (UNVERIFIED/unregistered dyno test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on framewelder-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible engine-displacement -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:engine-displacement-cc 99999.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible weld-defect-rate reading -> HARD hold ==")
    (println (exec-op actor "t14"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:weld-defect-rate-percent 999.0}}
                       coordinator))

    (println "== log-production-batch batch-001 attempting to self-issue an ECE R78/R40 type-approval certification -> HARD hold, PERMANENT ==")
