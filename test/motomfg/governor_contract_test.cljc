(ns motomfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control welding/assembly/test-bench
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue an ECE R78/ECE R40
  motorcycle type-approval certification') implemented faithfully.
  The single invariant under test:

    MotoAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Motorcycle Plant Operations
    Governor would reject; `:schedule-maintenance`/
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :sport-motorcycle}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :sport-motorcycle (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "framewelder-001" :maintenance-type :torch-inspection
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-welding-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "dynobench-002" :maintenance-type :load-cell-calibration
                             :scheduled-date "2026-08-01" :actuate-equipment? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the ECE R78/R40 type-approval authority"
    (let [[db actor] (fresh)
...
(deftest invalid-product-category-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-category :hoverbike}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-category} (-> (store/ledger db) last :basis)))
    (is (not= :hoverbike (:product-category (store/batch db "batch-001"))) "fabricated product-category never lands in the SSoT")))

(deftest invalid-engine-displacement-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:engine-displacement-cc 99999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-engine-displacement} (-> (store/ledger db) last :basis)))
    (is (not= 99999.0 (:engine-displacement-cc (store/batch db "batch-001"))) "fabricated engine-displacement never lands in the SSoT")))

(deftest invalid-defect-rate-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:weld-defect-rate-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-defect-rate} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:weld-defect-rate-percent (store/batch db "batch-001"))) "fabricated weld-defect-rate never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "framewelder-001" :severity :moderate
                                            :description "frame weld anomaly, brake mount misalignment"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "framewelder-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
